...
export default function InputSelect({options, onChange, initValue}: Props) {
    ...
    return (
        <div onClick={toggleOptions} className='select-none py-2 w-full dark:bg-dark-grey relative border border-input-default-color rounded-md text-paragraph-medium'>
            ...
            <div data-testid="options-wrapper" className={`${isOptionsShow ? '' : 'hidden'} py-2 bg-white border dark:border-line-dark-color dark:bg-very-dark-grey rounded-lg overflow-hidden w-full absolute bottom-12 md:top-12 md:bottom-auto left-0 text-primary-heading-color dark:text-primary-dark-heading-color`}>
                {
                    options.map((item, i) => {
                        return (
